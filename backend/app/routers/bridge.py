...
_CONTENT_REQUIREMENTS = (
    f"{settings.primary_user_name}'s standing content requirements for investment memos "
    f"(confirmed directly with {settings.primary_user_name}, apply these by default in every memo "
...
def _resolve_ticker_for_save(
        ...
        if new_ticker_status not in ticker_registry.STATUSES:
            return {
                ...
                "message": (
                    ...
                    + _addressing(
                        identity,
                        f"Ask {settings.primary_user_name} which bucket it belongs in",
    ...
    if resolution["kind"] == "confirm_needed":
        ...
        return {
            ...
            "message": (
                ...
                + _addressing(
                    identity,
                    f"Ask {settings.primary_user_name} whether they meant one of those, or this is genuinely a new ticker",
                    "Did you mean one of those, or is this genuinely a new ticker",
                )
                + f" — this tool won't guess. If {settings.primary_user_name} confirms it's genuinely new (not a typo "
                "of any of those), call save_final again with confirmed_new_ticker=True and "
                "new_ticker_status set — never set confirmed_new_ticker=True without them "
    ...
    return {
        ...
        + _addressing(
            identity, f"Ask {settings.primary_user_name} to confirm the exact ticker.", "Please confirm the exact ticker."
        ),
    }
...
def _resolve_ticker_folder(identity: str, ticker: str) -> str | dict:
    ...
    if resolution["kind"] == "confirm_needed":
        ...
        return {
            ...
            + _addressing(identity, f"Ask {settings.primary_user_name} to confirm.", "Please confirm."),
        }
...
def _build_server(identity: str) -> MCPServer:
    ...
    server = MCPServer(
        ...
        instructions=(
            ...
            f"own initiative. Only call send_report after {settings.primary_user_name} has seen that exact "
            ...
            f"after {settings.primary_user_name} has explicitly approved a specific version as final. A request "
            ...
            f"larger, or you're unsure, tell {settings.primary_user_name} to download the file themselves from "
            ...
            f"memo. Only reach for encoding='base64' on save_final when {settings.primary_user_name} "
    ...
    _send_report_description = (
        "Send a draft/revision to the other AI for it to review before "
        f"anything is finalized. ONLY call this after {settings.primary_user_name} has already "
        ...
        f"stop; wait for {settings.primary_user_name} to say to send it before calling this. "
    ...
    @server.tool(description=_send_report_description)
    def send_report(
    ...
    _get_pending_description = (
        ...
        f"this once is always enough — you never need to ask {settings.primary_user_name} \"is this "
        f"pending or older\" or call it a second time to see something they "
        "already looked at before. Each message includes `timestamp_et` "
        f"(already converted to {settings.primary_user_name}'s Eastern time, human-readable — "
        "use this one, not the raw UTC `timestamp` field) — always mention "
        f"when you tell {settings.primary_user_name} what you found (e.g. \"ChatGPT sent this Aug 19 "
        ...
        "instead.\n\n"
        f"IMPORTANT — {settings.primary_user_name}'s standing content requirements for investment "
        f"memos (confirmed directly with {settings.primary_user_name}, apply these by default in "
        "every memo unless they say otherwise), relevant to the rest of "
    ...
    @server.tool(description=_get_pending_description)
    ...
    _start_research_description = (
        f"Call this the moment {settings.primary_user_name} asks for one of their standard report "
        "types on a ticker (e.g. \"build me an investment memo on X\", \"give "
        f"me a competitive landscape analysis on Y\") — it returns {settings.primary_user_name}'s "
        "own research brief for that report type as `prompt`, with the "
        "ticker already filled in. Follow that brief to draft the report "
        "in this chat; it already includes their standing content/citation "
        ...
        f"doesn't match one of these — never guess which one {settings.primary_user_name} meant."
    )

    @server.tool(description=_start_research_description)
    ...
    _list_tickers_description = (
        f"List every ticker {settings.primary_user_name} currently has a folder for — use this "
    ...
    @server.tool(description=_list_tickers_description)
    ...
    @server.tool(description=_list_ticker_files_description)
    ...
    _read_ticker_file_description = (
        ...
        f"with {settings.primary_user_name} or ask them to describe what's in it rather than "
        "guessing at its contents."
    )

    @server.tool(description=_read_ticker_file_description)
        ...
        if text is None:
            ...
            return {
                ...
                "message": (
                    ...
                    + _addressing(
                        identity,
                        f"Share this link with {settings.primary_user_name}, or ask them what's in it: {preview_url}",
    ...
    _save_final_description = (
        f"Save {settings.primary_user_name}'s approved final version to a ticker's real Dropbox "
        ...
        f"this returns status='error' asking you to confirm with {settings.primary_user_name} which "
        ...
        f"refuses (status='error') and asks {settings.primary_user_name} to confirm the exact "
        f"ticker — it will not silently create a near-duplicate of a real "
        f"one. If {settings.primary_user_name} explicitly confirms it's genuinely a different, "
        "new ticker (not a typo), call save_final again with "
        "confirmed_new_ticker=True and new_ticker_status set, to actually "
        "create it — never set confirmed_new_ticker=True without them "
        ...
        f"so {settings.primary_user_name} gets something readable and well-formatted, not raw "
        ...
        f"Model.xlsx\") when {settings.primary_user_name} specifically needs a real spreadsheet "
        ...
        f"IMPORTANT — what {settings.primary_user_name} approved in this chat is NOT what they'll "
        "get. This tool re-renders `content` into this system's own "
        "formatted PDF, which looks different from whatever you showed "
        f"them here (different fonts, colors, layout) — so their earlier "
        ...
        f"draft until {settings.primary_user_name} has looked at `preview_url` specifically and "
        f"confirmed they're happy with THAT — not with what you showed them "
        "in chat. If they ask for changes, save again with "
        ...
        f"rather than saving without a clear recommendation {settings.primary_user_name} would "
    ...
    @server.tool(description=_save_final_description)
    def save_final(
        ticker: str,
        filename: str,
        content: str,
